...
class SVQ_backbone(nn.Module):
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        # model
        z, loss = self.backbone(z)                                               # z: [bs x nvars x d_model x patch_num]
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        # Encoder
        z, loss = self.encoder(u)                                                      # z: [bs * nvars x patch_num x d_model]
    ...
    def forward(self, src:Tensor, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None):
        ...
        if self.res_attention:
            for mod in self.layers:
                output, scores, loss = mod(output, prev=scores, key_padding_mask=key_padding_mask, attn_mask=attn_mask)

            return output, loss
        else:
            for mod in self.layers: 
                output, loss = mod(output, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
