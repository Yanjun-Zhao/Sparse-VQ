...
from layers.sparselinear import SparseLinear
from layers.ActivationSparsity import ActivationSparsity
...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        #self.sparselinear = SparseLinear(codebook_size, codebook_size, sparsity=0.9)
    ...
    def forward(
        ...
        #pdb.set_trace()
        #ind:codebook_index  onehot:distance matrix(onehot)
        embed_ind, embed_onehot = self.gumbel_sample(dist, dim = -1, temperature = sample_codebook_temp, training = self.training)
        ...
        #return output, embed_ind, dist
        return quantize, embed_ind, dist, embed
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        self.sparselinear = SparseLinear(dim, dim, sparsity=0.9)
    ...
    def forward(
        ...
        quantize, embed_ind, distances, codebook = self._codebook(x, sample_codebook_temp = sample_codebook_temp)
        
        #sparse
        quantize = self.sparselinear(quantize) + quantize + x
        ...
        if self.training:
            ...
            maybe_detach = torch.detach if not self.learnable_codebook else identity #identity
